      ...
      <main class="flex-1 overflow-y-auto p-4 md:p-6">
        <div class="bento-grid">
          ...
          @if (!facade.isLoading() && facade.recommendedProducts().length > 0) {
            <div class="bento-wide card flex flex-col gap-3 border-brand/30 bg-brand/5 relative overflow-hidden">
              <div class="absolute -right-4 -top-4 opacity-10">
                <app-icon name="sparkles" [size]="120" class="text-brand" />
              </div>
              
              <div class="z-10 flex gap-4 items-center">
                <div class="w-12 h-12 rounded-full bg-brand/20 flex items-center justify-center shrink-0">
                  <app-icon name="sparkles" [size]="24" class="text-brand" />
                </div>
                <div>
                  <h3 class="font-bold text-primary">Es momento de reponer</h3>
                  <p class="text-sm text-muted">Detectamos {{ facade.recommendedProducts().length }} productos que no compras hace más de una semana.</p>
                </div>
              </div>
                    ...
                    <div class="flex items-center gap-2">
                      <div class="relative w-24">
                        ...
                        @if (savedId() === product.id) {
                          <app-icon name="check" [size]="14" class="absolute right-2 top-1/2 -translate-y-1/2 text-brand" />
                        }
                      </div>
                    </div>
                  </div>
...
export class ProductsPage implements OnInit {
  ...
  async onPriceBlur(productId: string, event: Event) {
    ...
    if (!isNaN(newPrice)) {
      await this.facade.updatePrice(productId, newPrice);
  ...
  async generateSmartList() {
    await this.facade.generateSmartList();
